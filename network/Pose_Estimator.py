...
from network.fs_net_repo.PoseNet9D import PoseNet9D, PoseNet9D_save
...
class Pose_Estimator(nn.Module):
    def __init__(self, train_stage):
        ...
        if FLAGS.save_info == 0.0:
            self.posenet = PoseNet9D()
        else:
            self.posenet = PoseNet9D_save()
    ...
    def forward(self,  PC=None, depth=None, obj_id=None, camK=None,
        ...
        if FLAGS.use_clip==1.0:
            ...
            use_clip = False
        if FLAGS.use_clip_global==1.0:
            use_clip_global = True  
        else:
            use_clip_global = False
        if FLAGS.use_clip_nonLinear == 1.0:
            use_clip_nonLinear = True
        else:
            use_clip_nonLinear = False
